...
if __name__ == "__main__":
    ...
    # Load data
    data = sio.loadmat('./data/samechannel')
    channel_h = data['input_h']
    channel_g = sio.loadmat('./data/samechannel')['input_g']
    ...
    for i in range(3000):
        ...
        print("Action:============================",m_list1[np.argmax(r_list1)],type(m_list1[np.argmax(r_list1)]))
        # encode the mode with largest reward
        try:
    ...
    save_to_txt(Bat,"Battery")

    print("number", number)
    save_to_txt(Energy_change,"Energy_change")
